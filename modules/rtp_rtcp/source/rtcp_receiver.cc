...
struct RTCPReceiver::PacketInformation {
  ...
  int32_t receiver_diff_information = 0;
...
RTCPReceiver::RTCPReceiver(const RtpRtcpInterface::Configuration& config,
    ...
    : clock_(config.clock),
      ...
      // mustang add it
      rtcp_diff_observer_(config.differ_callback),
      // mustang end it
...
void RTCPReceiver::HandlePsfbApp(const CommonHeader& rtcp_block,
  ...
  {
    rtcp::Dfb dfb;
    if (dfb.Parse(rtcp_block)) {
      packet_information->packet_type_flags |= kRtcpDfb;
      packet_information->receiver_diff_information=dfb.get_diff();
      return;
    }
  }
  // mustang end it
...
void RTCPReceiver::TriggerCallbacksFromRtcpPacket(
  ...
  // mustang add it
  if (rtcp_diff_observer_) {
    //RTC_DCHECK(!receiver_only_);
    if (packet_information.packet_type_flags & kRtcpDfb) {
      RTC_LOG(LS_VERBOSE)
          << "Incoming DFB from SSRC " << packet_information.remote_ssrc;
      RTC_LOG(LS_INFO)
          << "Incoming DFB diff = "<< packet_information.receiver_diff_information;
      rtcp_diff_observer_->OnReceivedDiffRequest(packet_information.receiver_diff_information);
    }
  }
  // mustang end it
