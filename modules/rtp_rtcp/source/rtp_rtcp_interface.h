...
class RtpRtcpInterface : public RtcpFeedbackSenderInterface {
 public:
  struct Configuration {
    ...
    // mustang add it
    // Called when the receiver requests an Differ request
    RtcpDiffObserver * differ_callback = nullptr; 
    // mustang end it
  ...
  // Sets sending status. Sends kRtcpBmustangode when going from true to false.
  // Returns -1 on failure else 0.
  ...
  // mustang add it 
  virtual int32_t SendDIFB(int32_t diff_info){return 1;}
  // mustang end it 
  ...
  // mustang add it
  void SendDiffFeedback(int32_t diff_info) { SendDIFB(diff_info); }
  // mustang end it
