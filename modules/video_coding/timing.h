...
class VCMTiming {
 public:
  std::vector<int32_t> diff_recorder;
  int32_t diffinfo;
  ...
  // currently is `now_ms`.
  virtual int64_t RenderTimeMs(uint32_t frame_timestamp, int64_t now_ms);
 ...
 protected:
  ...
  int64_t RenderTimeMsInternal(uint32_t frame_timestamp, int64_t now_ms)
