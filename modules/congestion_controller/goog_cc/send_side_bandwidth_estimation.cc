...
void SendSideBandwidthEstimation::UpdateEstimate(Timestamp at_time) {
  ...
  if (LossBasedBandwidthEstimatorV2ReadyForUse()) {
    RTC_LOG(LS_INFO) << "RTC is using LossBasedBWEV2";
    uint8_t fraction_loss = this->fraction_loss();
    float loss_rate = static_cast<float>(fraction_loss) / 255.0;
    TimeDelta round_trip_time = this->round_trip_time();
    float rtt_ms = static_cast<float>(round_trip_time.ms());
    DataRate new_bitrate =
