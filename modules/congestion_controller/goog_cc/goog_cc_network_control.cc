...
void GoogCcNetworkController::OnDiffFeedbackFlag(DiffUpadte diffmsg){
  ...
  if(diff_recorder.size() < 120)
    ...
    diff_recorder.erase(diff_recorder.begin());
    diff_recorder.push_back(diffmsg.diff);
  };
  // 队列小于10，直接break
  if (diff_recorder.size() < 10) 
    return;
  int counter = 0;
  for (size_t i = 0; i < diff_recorder.size(); i++) {
    if (diff_recorder[i] < 0)
      counter++;
  }
  double now_ratio = counter / diff_recorder.size();
  if (now_ratio > diff_ratio) {
    diff_state = false;
    diff_ratio = diff_ratio * diff_backoff;
    diff_ratio = std::max(0.1, diff_ratio);
    last_ratio = diff_ratio;
    diff_counter = 0;
  }
  else {
    diff_state = true;
    diff_counter++;
    diff_ratio = last_ratio + diff_k * std::pow(diff_counter, 3);
    diff_ratio = std::min(diff_ratio, 0.25);
  }
  RTC_LOG(LS_INFO) << "gcc diff = " << diffmsg.diff << " diff_state = " << diff_state;
}
// mustang end it
...
void GoogCcNetworkController::MaybeTriggerOnNetworkChanged(
    ...
    Timestamp at_time) {
  auto acknowledged_bitrate = acknowledged_bitrate_estimator_->bitrate();
  DataRate avg_ack_bitrate = DataRate::Zero();
  if (ack_flag) {
    int current_ack_rate = 0;
    if (acknowledged_bitrate) {
      ack_recoder.emplace_back(acknowledged_bitrate.value());
      if (ack_recoder.size() >= 50) {
        for (auto ack_rate : ack_recoder) {
          if (avg_ack_bitrate.bps() == 0)
            avg_ack_bitrate = ack_rate;
          else 
            avg_ack_bitrate = 0.8 * avg_ack_bitrate + 0.2 * ack_rate;
        }
      }
      current_ack_rate = acknowledged_bitrate.value().bps();
    }
    if (current_ack_rate != 0 && ack_recoder.size() >= 50 && current_ack_rate < avg_ack_bitrate.bps() * ack_sigma) {
      ack_state = true;
    }
    else {
      ack_state = false;
    }
    if (ack_recoder.size() > 100) 
      ack_recoder.erase(ack_recoder.begin());
  }
  uint8_t fraction_loss = bandwidth_estimation_->fraction_loss();
  // RTT
  TimeDelta round_trip_time = bandwidth_estimation_->round_trip_time();
  // mustang get information
  DataRate drl_bitrate = bandwidth_estimation_->target_rate();
  absl::optional<DataRate> drl_ackrate = bandwidth_estimation_->get_ackrate();
  // get ackrate
  // mustang end get information
  // sender side bandwidth estimation
  DataRate loss_based_target_rate = DataRate::Zero();
  auto nowrate = bandwidth_estimation_->target_rate();
  if (diff_flag == true && diff_state == true && ack_flag== true && ack_state == false) {
      loss_based_target_rate = nowrate > last_target_rate ? nowrate: last_target_rate;
      last_target_rate = loss_based_target_rate;
  }
  else if (diff_flag== true && diff_state == true && ack_flag== true && ack_state == true) {
      std::cout << "avg ack rate = " << avg_ack_bitrate.bps() << std::endl;
      loss_based_target_rate = nowrate > avg_ack_bitrate? nowrate: avg_ack_bitrate;
      last_target_rate = loss_based_target_rate;
  }
  else {
    loss_based_target_rate = nowrate;
    last_target_rate = loss_based_target_rate;
  }
  std::cout << "gcc-like cca bitrate = " << loss_based_target_rate.bps() << std::endl;
