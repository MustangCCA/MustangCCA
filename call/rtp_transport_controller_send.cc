...
RtpTransportControllerSend::RtpTransportControllerSend(
    ...
    : clock_(clock),
      ...
      #ifdef CCA_PCC
      controller_factory_fallback_(
        std::make_unique<PccNetworkControllerFactory>(predictor_factory)),
      #endif
...
// mustang add it
RtcpDiffObserver* RtpTransportControllerSend::GetDiffObserver() {
  return this;
}
// mustang end it
...
// mustang add it
void RtpTransportControllerSend::OnReceivedDiffRequest(int32_t diff){
  DiffUpadte diffmsg;
  diffmsg.diff_time = Timestamp::Millis(clock_->TimeInMilliseconds());
  diffmsg.diff=diff;
